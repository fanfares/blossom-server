...
const AUDIO_CODEC_FOR_FORMAT = {
  mp4: ["aac", "mp3"],
  mkv: ["aac", "mp3"],
  webm: ["vorbis", "opus"],
} as const;

const VideoOptimizeSchema = z
  .object({
    quality: z
      .number()
      .int()
  ...
  .superRefine((v, ctx) => {
    ...
    if (!validVideo.includes(v.videoCodec)) {
      ctx.addIssue({
    ...
    if (!validAudio.includes(v.audioCodec)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["audioCodec"],
        message:
...
export const ConfigSchema = z
  .object({
    ...
    media: MediaSchema.optional()
      .transform((v) => v ?? MediaSchema.parse({}))
