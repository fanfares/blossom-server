...
| `paidStorage.cashu.mintUrl`             | `https://mint.macadamia.cash` | Cashu mint used as the Lightning invoice and settlement-status intermediary                                                                                                                                                                  |
...
| `POST` | `/storage/purchases`     | Create a capacity-duration or active-grant extension invoice |
...
New capacity purchases separate storage size from time. For example,
`{ "purchaseType": "new", "storageUnits": 2, "durationYears": 3 }` reserves 2
GiB for three years and charges six GiB-years. To extend every active grant
without adding duplicate capacity, use
`{ "purchaseType": "extension", "durationYears": 2 }`. Extension targets are
snapshotted when the invoice is created and expiry updates are idempotent.

When treasury forwarding is enabled, the server claims each settled Cashu mint
quote and pays the configured Lightning Address. Customer quota is activated
first; payout state is stored in a durable outbox and retried without creating
duplicate payments after temporary failures or restarts. The destination
receives the purchase amount minus the Cashu mint's Lightning routing fee. The
quota grant, payment, and forwarding outbox live in libSQL, so Cloudflare
Container deployments must use persistent Turso rather than container-local
SQLite.
